    ...
    for sentence in range(0, len(testfeat)):
        ...
        for word in range(0, len(testfeat[0])-1):
            if testfeat[sentence][word]==1:
                ptrue+=log(parameters[word][3])
                pfalse+=log(parameters[word][2])
            elif testfeat[sentence][word]==0:
                ptrue+=log(parameters[word][1])
                pfalse+=log(parameters[word][0])
...
testFeatures=[]
makeFeaturesSet('trainingSet.txt', vocab, testFeatures)
