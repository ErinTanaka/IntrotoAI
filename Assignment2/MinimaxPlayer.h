...
public:
     ...
     */
    MinimaxPlayer* clone();
		OthelloBoard* getSuccessors(OthelloBoard* currentBoard, char symb);
		int utilityFn(OthelloBoard * currentBoard);
		void successorCoor(OthelloBoard* currentBoard, char symb, int ** coorList);
		int miniMaxFn(OthelloBoard* b, int ** succArray, int len, char symbol);
private:
